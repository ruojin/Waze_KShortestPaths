...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCJGzMTRCabD_HZawX2rFAGPyA8aMHdQQ8&libraries=visualization&callback=initMap">
    </script>
<script>
	...
	function initList(){				//	send XMLHttpRequest() to get nodes and edges info in a XML form
		...
		xhr.open("get","WazeAppServlet?NodeID=&NodeID1=",true);
...
//        	    icon:'https://maps.gstatic.com/intl/en_us/mapfiles/markers2/measle_blue.png',			//	little blue dot icon
//        	    icon: 'https://www.google.com/mapfiles/marker_yellow.png',
        	...
        	google.maps.event.addListener(marker, 'click', function(){
        		if(flag_Direction==0){									//	Remove the highlighted roads when click on another marker
    				...
    				if(marker_temp!=null){
 //   					marker_temp.setIcon('https://www.google.com/mapfiles/marker_yellow.png');
...
/*         			    content: '<p>NodeID:' + split[0].split(':')[1] + '</p>' +
        			    		'<p>Latitude:' + split[1].split(':')[1] + '</p>' +
        			    		'<p>Longitude:' + split[2].split(':')[1] + '</p>' */
/*         			    content:'NodeID   :' + split[0].split(':')[1] + '<br>' +
        			    		'Latitude :' + split[1].split(':')[1] + '<br>' +
        			    		'Longitude:' + split[2].split(':')[1] + '<br>' */
...
//	        				marker1.setIcon('https://www.google.com/mapfiles/marker_yellow.png');
	        				marker1.setIcon('https://storage.googleapis.com/support-kms-prod/SNP_2752125_en_v0');
	        				var NodeID2 = document.getElementById("NodeID2").value;
	        				var marker2 = markers[NodeID2];
//	        				marker2.setIcon('https://www.google.com/mapfiles/marker_yellow.png');
...
//        				Directions_Nodes[1] = this;						//	Store selected marker (zoom out/in to fit both nodes into the map)
	...
	function HighlightRequest(){					
		...
		xhr2.open("get","WazeAppServlet?NodeID="+NodeID+"&NodeID1=",true);	//	Pass the value of NodeID to Servlet
...
/* 		document.getElementById("map").addEventListener("click", function(){		//	Add a listener to clear the adjacent
			polyline.setMap(null);													//	edges of the last selected node
			var marker = markers[NodeID];					// Change back the color of marker
			marker.setIcon('https://www.google.com/mapfiles/marker_yellow.png');
			marker.setMap(null);							// Delete certain marker
 			marker = new google.maps.Marker({				// create a marker and set id
        		id: NodeID,
...
//				marker.setIcon('https://www.google.com/mapfiles/marker_yellow.png');	//	Reset color to yellow
...
//				marker1.setIcon('https://www.google.com/mapfiles/marker_yellow.png');	//	Reset color to yellow
				marker1.setIcon('https://storage.googleapis.com/support-kms-prod/SNP_2752125_en_v0');
				Direction_infoWindows[0].close();
			}
...
//				marker2.setIcon('https://www.google.com/mapfiles/marker_yellow.png');	//	Reset color to yellow
				marker2.setIcon('https://storage.googleapis.com/support-kms-prod/SNP_2752125_en_v0');
				Direction_infoWindows[1].close();
			}
			ClearSelectedInfo();							//	Clear selected node TextFields
...
//				marker2.setIcon('https://www.google.com/mapfiles/marker_yellow.png');
	...
	function ShortestRequest(){
		...
		var NodeID1 = document.getElementById("NodeID1").value;
		var NodeID2 = document.getElementById("NodeID2").value;
		xhr3.open("get","WazeAppServlet?NodeID=&NodeID1="+NodeID1+"&NodeID2="+NodeID2,true);	//	Pass the value of NodeID to Servlet
...
//					$('#Distance1').val(distance);
				...
				if (i!=0 && p==1) {
//					$('#Distance2').val(distance.toFixed(6));
				...
				if (i!=0 && p==2) {
//					$('#Distance3').val(distance.toFixed(6));
